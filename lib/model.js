...
export async function callModelJson({ system, user, temperature = 0.4, nodeConfig = null, enableCache = true, maxTokens = null }) {
  ...
  const defaultTimeouts = { start: 20, directions: 15, choose: 25, book: 30, cover: 15 };
  const timeoutMs = Math.max(5, Number(nodeConfig?.timeoutSeconds || defaultTimeouts[nodeConfig?.key] || 20)) * 1000;
