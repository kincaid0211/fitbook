...
export function buildStartUserPayload({ parsed }) {
  return JSON.stringify({
    ...
    task: "理解用户起点内容，作为非书的第 1 章，提炼后续方向生成所需的结构化材料。",
    requirements: [
      "summary 一句话，30–80 字。",
...
export function buildBookUserPayload({ route, interestProfile }) {
  return JSON.stringify({
    ...
    task: "把已完成章节包装为一本非书。章节为导读式内容，不替代原文全文。",
    requirements: [
      ...
      "chapters[i].bridge 与 route[i].bridge 对齐;route[i] 没有 bridge（通常是第 1 章）时写 \"。",
      "chapters[i].curatorNote 一句策展点评。",
      "title 8–18 字书名，点出本书的主题或视角。",
      "subtitle 14–28 字副标题，呼应章节线索的风格。",
      "preface 100–220 字序言，说明这些章节怎么连起来、读者会得到什么。",
      "tags 3–6 个 2–6 字章节线索标签。",
      "style 6–18 字章节线索风格总结。",
      "explorationSummary 80–160 字探索总结，复盘读过的章节。",
