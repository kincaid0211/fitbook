...
export function buildStartUserPayload({ parsed, hotItem = null }) {
  return JSON.stringify({
    ...
    task: "你是起点定调节点。从用户给出的起点（文章、链接、粘贴文本或热榜条目）中，提取3-6张「知识卡片」。每张卡片是一个值得深入探索的知识切入点——不是全文摘要，而是一个可以生长出后续章节的'种子'。用户必须选择其中一张作为整本非书的「锚点」，后续探索将围绕锚点展开。请基于实际可获取的材料进行提炼，不编造未提及的细节。",
    requirements: [
