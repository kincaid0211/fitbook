...
- Online sharing (2026-05-14): Finished books can be shared via short links (`#s=<id>`) backed by server-side storage (Vercel KV). The receiver opens the link and the frontend fetches the full book data from `GET /api/share/<id>`. If online sharing is unavailable, the system falls back to legacy compressed URL hashes (`#book=`). Old `#book=` links remain compatible.
